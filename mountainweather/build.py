...
/* Heading styles applied directly to summary to fix Kindle double-height bug */
...
with open("trmnl.html", "w", encoding="utf-8") as f: f.write(trmnl_tmpl)                elif "How Cold?" in l: sec = "temp"
                elif "Freezing Level" in l: sec = "freezing_level"
                elif any(x in l for x in ["Summary", "Effect", "Sunshine", "Planning"]): sec = "ignore"
                elif sec == "date":
                    for d in ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]:
